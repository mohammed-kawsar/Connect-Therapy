...
class PatientPreviousNotesViewTest(TestCase):
    def setUp(self):
        test_user_1 = User.objects.create_user(username='testuser1')
        test_user_1.set_password('12345')
        test_user_1.save()
        ...
        self.appointment = Appointment(patient=self.patient,
                                       practitioner=self.practitioner,
                                       start_date_and_time=datetime(year=2016,
                                                               month=4,
                                                               day=17,
                                                               hour=15,
                                                               minute=10),
    ...
    def test_test_func_when_user_has_no_patient(self):
        factory = RequestFactory()
        request = factory.post(reverse_lazy('connect_therapy:patient-view-notes',
                                            kwargs={'pk': 1}))
        request.user = AnonymousUser()
        view = PatientPreviousNotesView()
        view.request = request
        self.assertFalse(view.test_func())
    ...
    def test_get_context_data(self):
        factory = RequestFactory()
        request = factory.get(reverse_lazy('connect_therapy:patient-view-notes',
                                            kwargs={'pk': 1}))
        view = PatientPreviousNotesView()
        view.request = request
        view.object = self.appointment
